# https://timeseries-ru.github.io/course/recipes_book.html
...
# с Excel-файлами (при наличии некоторых предустановленных библиотек по типу xlrd - если их нет, будет ошибка)
...
# отсмотреть распределение как целевой величины
sns.distplot(
    dataframe.value,
    label='VALUE')
sns.distplot(
    ...
    label='TARGET',
    color='red')
# точка с запятой в конце вывода графики нужна, чтобы не отображать объект диаграммы в выводе ячейки
plt.legend(loc='best')
...
X_train, X_test, y_train, y_test, z_train, z_test, indices_train, indices_test = train_test_split(
    ...
    stratify=z  # сохранять пропорции классов в разбиениях также по умолчанию этот метод перемешивает данные
)
...
# при желании, можно добавить колонку и подогнать и преобразовать сразу
...
"silhouette score %.3f" % silhouette_score(pipeline.transform(X), clusterer.labels_)
