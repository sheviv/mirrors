...
"""
Доступ к общим ресурсам (lock) - не позволяет сразу нескольким потокам работать с одними и теми же данными.
"""
# acquire() - позволяет потоку получить блокировку. 
# release() - позволяет вызывать его из любого потока, а не только из потока, который заблокировал Lock в данный момент.
# lock = threading.Lock()
# x = 'Рython 2'
# lock.acquire()
# x = 'Рython 3'
# print(x)
# lock.release()  # 'Рython 3'
